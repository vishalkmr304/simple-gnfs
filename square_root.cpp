...
void find_roots(Polynomial &polynomial)
{
   ...
   root = NTL::MakeRR(power(polynomial.f.rep[2], 2) - 3 * 
		(polynomial.f.rep[1]), 0);
   root = SqrRoot(NTL::abs(root));

   val = (-NTL::MakeRR(polynomial.f.rep[2], 0) + root) / 3 + 10;
   polynomial.roots.push_back(Newton(polynomial, val));

   val = (-NTL::MakeRR(polynomial.f.rep[2], 0) - root) / 3 - 10;
   polynomial.roots.push_back(Newton(polynomial, val));

   val = -NTL::MakeRR(polynomial.f.rep[2], 0) / 3;
   polynomial.roots.push_back(Newton(polynomial, val));
...
void factorizeAFB(int a, int b, std::vector<int> &AFB, std::vector<int> &AFBr,
   int u, int *AFBvec, Polynomial &polynomial)
{
   NTL::ZZ valZ;
...
// {{{ square_root()
void square_root(Polynomial &polynomial, Target &target, Matrix &matrix,
   ...
   NTL::ZZ &xZ, NTL::ZZ &yZ)
{
   ...
   // find complex roots of f(x)
	find_roots(polynomial);
   ...
   for(int number = -1; number < nRows && !stop && attempt < 15; number++)
   {
      ...
      yZ = MulMod(dF(polynomial.f, polynomial.m), yZ,target.n);
      free(RFBvec);
      ...
      for(i = 0; i < 3; i++)
      {
         ...
         for(j = 0; j < pairs_needed; j++)
         {
            ...
            {
               product *= av[j] + (polynomial.roots[i]) * bv[j];
            }
         }
         product = SqrRoot(abs(product));
         b0 += delta_I(polynomial.f, 1, polynomial.roots[i])*product;
         b1 += delta_I(polynomial.f, 2, polynomial.roots[i])*product;
         b2 += delta_I(polynomial.f, 3, polynomial.roots[i])*product;
      ...
      if(polynomial.roots[0] == polynomial.roots[1])
      {
         product *= product;
      }
